/**
 * @file mp3_scan_band.cpp
 * @brief Scan all albums of a band and call mp3_scan_music for each MP3 file.
 *
 * Usage: mp3_scan_band.exe <band_path>
 *
 * Iterates over album subfolders (YYYY - Album Name) and for each MP3
 * file found, calls: mp3_scan_music.exe <full_mp3_path>
 */
...
int main(int argc, char* argv[]) {
    ...
    if (szArglist == NULL || nArgs < 2) {
        ...
        std::cout << "Scans all albums of a band and calls mp3_scan_music for each MP3." << std::endl;
        LocalFree(szArglist);
        return 1;
    }
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "       MP3 Scan Band" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    for (const auto& albumEntry : albums) {
        ...
        for (const auto& mp3Entry : mp3Files) {
            ...
            #ifdef _WIN32
            std::string cmd = "mp3_scan_music.exe \"" + mp3Path.string() + "\"";
            #else
            std::string cmd = "./mp3_scan_music \"" + mp3Path.string() + "\"";
            #endif
